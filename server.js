...
data.push(password);
 ...
 server.send({
  ...
  app.post('/showMenus',(req,res) =>{
    connection.query("select m.MENU_ID,m.MENU_NAME,s.SUBMENU_ID,s.SUBMENU_NAME from menus m right JOIN sub_menus s on m.MENU_ID=s.MENU_ID ",(err,result) => {   
    var userdata=result;
...
app.post('/insertRoleTabMenu',(req,res) => {
 ...
 for(var i=0;i<arr.length;i++)
 {
     ...
     var c=arr[i].MENU_ID;
     var z=arr[i].SUBMENU_ID;  
     var submenu=parseInt(z);
     var d=arr[i].EDIT;
     var edit=parseInt(d);
     var e=arr[i].VIEW;
     var view=parseInt(e);
     var f=arr[i].APPROVE;
     var approve=parseInt(f);
         

connection.query("insert into  vc_role_tab_menu (ROLE_ID,TAB_ID,MENU_ID,SUBMENU_ID,EDIT,VIEW,APPROVE,CREATED_BY,STATUS) values (?,?,?,?,?,?,?,?,?);",[a,b,c,submenu,edit,view,approve,1,1], (err, results) => {
    if (err) throw err;
...
app.post('/getRolesByIds', (req, res) => {
  console.log("in getUserByIds==>");
  var a = req.body.key;
  console.log("in getUserByIds==>" + a);
  connection.query("select role_id,role_name,role_description from role where role_id=?", [a], (err, results) => {
  if (err) throw err;
  ...
  console.log("in editusers==>");
  var parameters = [];
  var role_name = req.body.roleName;
  console.log("role_name" + role_name);
  var role_description = req.body.roleDescription;
  var id = req.body.key;
  console.log("idddd" + id);
  var query = "UPDATE role SET ";
  if (role_name != undefined && role_name != '') {
  query = query + "role_name = ?,";
  parameters.push(role_name);
  }
  if (role_description != undefined && role_description != '') {
  query = query + "role_description = ?,";
  parameters.push(role_description);
  }
  query = query.substr(0, query.length - 1);
  query = query + " where role_id = ?";
  parameters.push(id);
  console.log(query);
