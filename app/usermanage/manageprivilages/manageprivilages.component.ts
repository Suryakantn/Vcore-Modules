...
@Component({
  selector: 'app-manageprivilages',
  templateUrl: './manageprivilages.component.html',
  styleUrls: ['./manageprivilages.component.css']
})
export class ManageprivilagesComponent implements OnInit {
  user:Registers;
  permision:Registers;
  public menuid;
  priveilege:Registers;
  userData: any = {};
  public SubMenus=[];
  public privilages=[];
...
//  perission1=[];
 arr=[];
  constructor(private router: Router,private usermanage:UserroleService,private route: ActivatedRoute) { }

  ngOnInit() {
    this.showMenusData();

    var data = this.route.snapshot.paramMap.get('roleid');
    this.userData.key = data;
    alert("id="+this.userData.key);
    this.usermanage.getRolesById(this.userData).subscribe(data => {
  ...
  showMenusData()
  {
    this.usermanage.showMenus1(this.user).subscribe((data1) =>{
...
onSubmitClk()
{
  ...
  for(var i=0;i<this.SubMenus.length;i++)
  {
   ...
   var pdata=strData.split("/");
    ...
    this.usermanage.insertRoleTabMenu(this.Role_Tab_menu).subscribe(data=>{
    alert(data);
    });
}
